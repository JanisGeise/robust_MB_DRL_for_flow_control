...
def predict_trajectories(env_model_cl_p: pt.nn.Module, env_model_cd: pt.nn.Module, episode: int,
                         ...
                         len_trajectory: int = 400) -> dict or pt.Tensor:
    """
    predict a trajectory based on a given initial state and action using trained environment models for cd, and cl-p

    :param env_model_cl_p: the trained environment model for cl anf p
    :param env_model_cd: the trained environment model for cd
    ...
    env_model_cl_p.load_state_dict(pt.load(f"{path}/cl_p_model/bestModel_train.pt"))
    env_model_cd.load_state_dict(pt.load(f"{path}/cd_model/bestModel_train.pt"))
    ...
    for t in range(len_trajectory - n_input_steps):
        ...
        traj_cd[t + n_input_steps] = env_model_cd(feature).squeeze().detach()

        # make prediction for probes and cl
        prediction_cl_p = env_model_cl_p(feature).squeeze().detach()
...
def train_env_models(path: str, n_t_input: int, n_probes: int, observations: dict, n_neurons: int = 50,
    ...
    print("start training the environment models")
    ...
    if load:
        env_model_cl_p.load_state_dict(pt.load("".join([path, "/cl_p_model/", f"bestModel_train.pt"])))
        env_model_cd.load_state_dict(pt.load("".join([path, "/cd_model/", f"bestModel_train.pt"])))
    ...
    train_loss_cd, val_loss_cd = train_model(env_model_cd, observations["states_train"], observations["actions_train"],
                                             ...
                                             batch_size=batch_size, cd_model=True)

    return env_model_cl_p, env_model_cd, [[train_loss, train_loss_cd], [val_loss, val_loss_cd]]


def fill_buffer_from_models(env_model_cl_p, env_model_cd, episode, path, observation, n_input, n_probes,
                            buffer_size, len_traj):       # cluster runs python 3.8... -> list[dict]:
    """
