...
def plot_influence_ratio(settings: dict, runtime: Tuple[pt.Tensor, list], reward: Tuple[pt.Tensor, list],
                         ratios: list) -> None:
    """
    ...
    ax.plot(ratios, runtime, color="red", label="run time $\left[t^*\\right]$")
    ax.plot(ratios, runtime + reward, color="black", label="$\sum\,\left(\\frac{1}{r^*}, r^*\\right)$")
    ax.set_ylabel("$mean$ $values$", usetex=True, fontsize=12)
...
def data_loader(settings: dict) -> dict:
    """
    loads all data, normalizes rewards, runtimes cl and to [0, 1]
    ...
    # scale everything to [0, 1], global max is determined by runtime, global min by either rewards or cl
    ...
    return {"mean_cl": cl, "mean_cd": cd, "mean_rewards": inverse_r, "runtimes": rt,
            "n_cases": len(settings["case_name"])}
...
if __name__ == "__main__":
    ...
    setup = {
        ...
        "path_controlled": r"drlfoam/examples/test_MF_vs_MB_DRL/influence_ratio_MF_MB/",  # main path to all the cases
        "case_name": ["all_MF/", "MB_MF_5/", "MB_MF_10/"],
        "avg_over_cases": False,                                # if cases should be averaged over, e.g. different seeds
        "ratio_MB_MF_episodes": [0, 5/1, 10/1],        # the ratios between episodes using env. model & CFD
        "runtimes": [25817, 11520, 5701]          # run times [s]
    }
